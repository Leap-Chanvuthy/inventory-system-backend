...
class PurchaseInvoiceAPIController extends Controller
{
    ...
    //             'supplier_id' => 'required|exists:suppliers,id',
    //             'payment_method' => 'required|string',
    //             'invoice_number' => 'required|string|unique:purchase_invoices,invoice_number,' . $id, // Exclude current invoice from unique check
    //             'payment_date' => 'nullable|date',
    //             'discount_percentage' => 'nullable|numeric',
    //             'tax_percentage' => 'nullable|numeric',
    //             'status' => 'required|string',
    ...
    //             // Ensure that raw materials belong to the supplier
    //             if ($rawMaterial->supplier_id !== $supplierId) {
    //                 throw new \Exception("Raw material with ID {$rawMaterialId} does not belong to the selected supplier with ID {$supplierId}.");
    //             }
    ...
    //         // Delete old PurchaseInvoiceDetail records
    //         PurchaseInvoiceDetail::where('purchase_invoice_id', $purchaseInvoice->id)->delete();

    //         // Insert new PurchaseInvoiceDetail records
    //         foreach ($rawMaterialsData as $material) {
    //             $material['purchase_invoice_id'] = $purchaseInvoice->id;
    //             PurchaseInvoiceDetail::create($material);
    //         }
    ...
    //             'message' => 'Purchase inovice updated successfully' ,
